...
from app.db.mysql import get_leave_request, insert_leave_request, get_leave_balance, cancel_leave_request
from app.deps import get_llm
from app.workflows.leave.models import LeaveState
from app.workflows.leave.rules import validate_leave
from app.prompts.LeavePrompts import leave_graph_prompts

SLOT_SYSTEM = leave_graph_prompts.SLOT_SYSTEM
SLOT_USER = leave_graph_prompts.SLOT_USER

TIME_SYSTEM = leave_graph_prompts.TIME_SYSTEM
TIME_USER = leave_graph_prompts.TIME_USER
...
# 确认意图（取消，查询，申请）
# todo: 将意图字符列表单独提出作为常量类保存
def decide_intent(state: LeaveState) -> str:
    """apply / query / cancel"""
    text = (state.get('text') or state.get('question') or '').lower()

    if any(k in text for k in ['cancel', 'cancelled', '取消', '撤销', '作废']):
        return 'cancel'

    if any(k in text for k in ['search', 'check', '查询', '查', '状态', '进度', '结果']):
...
def query_leave_node(state: LeaveState) -> dict:
    ...
    if not leave_id:
        # 若没有提供id，到数据库中查找此用户所有申请或前几个申请单据
        return {'answer': '请提供请假编号(如 LV-xxxxxxx)，以进行查询。'}
...
def cancel_leave_node(state: LeaveState) -> dict:
    ...
    ok = cancel_leave_request(leave_id) # 未实现的mysql方法
    if not ok:
...
def validate_node(state: LeaveState) -> dict:
    ...
    bal = get_leave_balance(requester) or {}    # 查看数据库内剩余假期，等待实现
